...
def enhancedPacmanFeatures(state, action):
    ...
    if maxFoodDistance:
        newheuristic = 1.0/(maxFoodDistance)
        newheuristic = newheuristic ** 2

    scaredList = []
    for st in newScaredTimes:
        scaredList.append(st)

    newheuristic += max(scaredList)
    heuristic = newheuristic
    
    features[action,'params'] = heuristic 
